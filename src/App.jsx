...
function Onboarding({ goto, onDiagnostic }) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchDiagnostico, minDelay]).then(([data]) => {
      if (cancelled) return;
      onDiagnostic(data && data.success ? data.diagnostico : null);
      goto("resultado");
    });
...
export default function EcoCheckApp() {
  ...
  return (
    <div className="ec-root">
      ...
      {view === "onboarding" && <Onboarding goto={goto} onDiagnostic={setDiagnostic} />}
      ...
      {view === "app" && (
        <AppShell page={page} setPage={setPage} currentUser={currentUser} onLogout={handleLogout}>
          {page === "dashboard" && <DashboardHome goto={goto} currentUser={currentUser} />}
          {page === "diagnostico" && <Resultado goto={goto} diagnostic={diagnostic} />}
          {page === "pendencias" && <PendenciasPage goto={goto} />}
