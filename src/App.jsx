...
import { supabase, salvarDiagnostico, carregarDiagnostico } from "./supabaseClient.js";
...
function Onboarding({ goto, onDiagnostic }) {
  ...
  // Mensagens de "Analisando… / Organizando…" — só visuais, sem depender da API.
  useEffect(() => {
    if (!loading) return;
  ...
  // Chama a IA de verdade (api/gerar-diagnostico) e só avança quando a resposta chegar
  // (ou falhar) — assim a tela nunca trava e nunca mostra "pronto" antes da hora.
  useEffect(() => {
    if (!loading) return;
    let cancelled = false;
    const minDelay = new Promise((r) => setTimeout(r, 2200));
    const fetchDiagnostico = fetch("/api/gerar-diagnostico", {
      method: "POST",
    ...
    Promise.all([fetchDiagnostico, minDelay]).then(([data]) => {
      if (cancelled) return;
      onDiagnostic(a, data && data.success ? data.diagnostico : null);
      goto("resultado");
    });
...
/* ============================== DIAGNOSTIC RESULT ============================== */
const ICON_BY_KEY = {
  licenciamento: ShieldCheck, documentos: FileText, residuos: Recycle,
  agua: Droplets, prazos: Clock,
};

function Resultado({ goto, diagnostic }) {
  const level = LEVEL_ORDER.includes(diagnostic?.nivelRisco) ? diagnostic.nivelRisco : LEVEL;
  const points = diagnostic?.pontosAtencao ?? POINTS;
  const resumo = diagnostic?.resumoCards?.length ? diagnostic.resumoCards : RESUMO_CARDS;
  const pendencias = diagnostic?.pendencias?.length ? diagnostic.pendencias : PENDENCIAS;
  const gerado = !!diagnostic;
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.paper }}>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "48px 24px 90px" }} className="ec-fade">
        ...
        <div style={{ textAlign: "center", fontSize: 14.5, color: C.ink, fontWeight: 600, marginBottom: 8 }}>
          Identificamos {points} pontos que merecem verificação.
        </div>
        {!gerado && (
          <div style={{ textAlign: "center", fontSize: 11.5, color: C.gray400, marginBottom: 16 }}>
            (Exibindo diagnóstico de demonstração — a IA não retornou um resultado desta vez.)
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12, marginBottom: 40 }} className="ec-resumo-grid">
          {resumo.map((r) => {
            const Icon = ICON_BY_KEY[r.key] || ClipboardList;
            const tone = TONE[r.tone] ? r.tone : "gray";
            return (
              <Card key={r.key} style={{ padding: 16, textAlign: "center" }}>
                <div style={{ width: 34, height: 34, borderRadius: 9, background: TONE[tone].bg, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 10px" }}>
                  <Icon size={16} color={TONE[tone].fg} />
                </div>
...
function traduzErroSupabase(err) {
  ...
  if (msg.includes("Failed to fetch")) return "Não foi possível conectar ao servidor. Verifique sua internet.";
...
function AuthPage({ mode, setMode, goto, onAuthed }) {
  ...
  const submit = async (e) => {
    ...
    if (mode === "signup") {
      ...
      setLoading(false);
      if (signUpError) { setError(traduzErroSupabase(signUpError)); return; }
      // Se a confirmação de e-mail estiver ativada no projeto Supabase,
      // data.session vem nulo até o usuário clicar no link do e-mail.
      if (!data.session) {
        setError("Conta criada! Verifique seu e-mail para confirmar antes de entrar.");
        setMode("login");
        return;
      }
      onAuthed({ name, company, email: data.user.email });
    } else {
      ...
      setLoading(false);
      if (signInError) { setError(traduzErroSupabase(signInError)); return; }
      const meta = data.user.user_metadata || {};
      onAuthed({ name: meta.name || data.user.email, company: meta.company || "", email: data.user.email });
    }
  };
    ...
    <FullScreenCenter>
      <div className="ec-fade" style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div className="ec-mono" style={{ textAlign: "center", fontSize: 10.5, color: C.gray400, marginTop: 14, lineHeight: 1.5 }}>
          Seus dados de acesso são protegidos pelo Supabase.
        </div>
      </div>
    </FullScreenCenter>
  );
...
/* ---- Dashboard Home ---- */
function EmptyDiagnosticState({ goto, firstName }) {
  return (
    <Card style={{ padding: 40, textAlign: "center", maxWidth: 480, margin: "40px auto" }}>
      <div style={{ display: "inline-flex", padding: 16, borderRadius: 999, background: C.greenSoft, marginBottom: 18 }}>
        <Sparkles size={26} color={C.green} />
      </div>
      <h3 className="ec-display" style={{ fontSize: 18, fontWeight: 700, color: C.forest, marginBottom: 8 }}>
        {firstName}, você ainda não fez seu diagnóstico.
      </h3>
      <p style={{ fontSize: 13.8, color: C.gray600, lineHeight: 1.6, marginBottom: 20 }}>
        Responda algumas perguntas simples sobre sua empresa e receba, em minutos, os pontos ambientais que merecem atenção.
      </p>
      <Btn icon={ArrowRight} onClick={() => goto("onboarding")}>Fazer diagnóstico gratuito</Btn>
    </Card>
  );
}

function DashboardHome({ goto, currentUser, diagnostic }) {
  ...
  if (!diagnostic) return (
    <div>
      ...
      <EmptyDiagnosticState goto={goto} firstName={firstName} />
    </div>
  );

  const pendencias = diagnostic.pendencias || [];
  const stats = [
    { n: String(pendencias.length), l: "pendências", tone: "orange" },
    { n: "—", l: "documentos (em breve)", tone: "gray" },
    { n: "—", l: "obrigações (em breve)", tone: "gray" },
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "290px 1fr", gap: 20, marginBottom: 24 }} className="ec-dash-top">
        <Card style={{ padding: "22px 20px" }}>
          <LevelBadge level={diagnostic.nivelRisco} points={diagnostic.pontosAtencao} size="sm" />
        </Card>
      ...
      <div style={{ display: "grid", gap: 12, marginBottom: 34 }}>
        {pendencias.length === 0 && <Card style={{ padding: 18, fontSize: 13.5, color: C.gray600 }}>Nenhuma pendência identificada até agora.</Card>}
        {pendencias.map((p) => (
          <Card key={p.id} style={{ padding: 18, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }} className="ec-dash-bottom">
        <div>
          ...
          <div style={{ fontSize: 11.5, color: C.gray400, marginTop: -28, marginBottom: 10 }}>(dados de demonstração — calendário real ainda não conectado)</div>
        ...
        <div>
          ...
          <div style={{ fontSize: 11.5, color: C.gray400, marginTop: -28, marginBottom: 10 }}>(dados de demonstração — documentos reais ainda não conectados)</div>
          <Card style={{ padding: 6 }}>
            {ultimosDocs.map((d, i) => (
...
/* ---- Pendências page ---- */
function PendenciasPage({ goto, diagnostic }) {
  const pendencias = diagnostic?.pendencias || [];
  const [openId, setOpenId] = useState(pendencias[0]?.id ?? null);

  if (!diagnostic) {
    return (
      <div>
        <PageHeader title="Pendências" sub="Pontos identificados no seu diagnóstico que ainda merecem verificação." />
        <EmptyDiagnosticState goto={goto} firstName="Você" />
      </div>
    );
  }
...
/* ---- Assistente IA (demo com respostas guiadas) ---- */
function AssistentePage({ goto }) {
  ...
  const reply = (userText) => {
    ...
    if (t.includes("licença") || t.includes("licenca") || t.includes("condicionante")) {
      return {
        ...
        actions: [{ label: "Ver licença", to: "licencas" }, { label: "Adicionar prazo", to: "calendario" }],
      };
    }
    if (t.includes("poço") || t.includes("poco")) {
      return {
        text: "Você informou que usa um poço, mas não há outorga cadastrada. Pode ser necessário regularizar o uso da água junto ao órgão gestor.",
        actions: [{ label: "Ver tutorial", to: "tutoriais" }, { label: "Adicionar prazo", to: "calendario" }],
      };
    }
    if (t.includes("falt") || t.includes("document")) {
      return { text: "Identificamos documentos que ainda não foram enviados.", actions: [{ label: "Ver documentos", to: "documentos" }] };
    }
    return { text: "Isso está relacionado às suas pendências de licenciamento e resíduos.", actions: [{ label: "Ver pendências", to: "pendencias" }] };
  };
  ...
  const send = (text) => {
    ...
    setInput("");
    setTyping(true);
    setTimeout(() => { setTyping(false); setMessages((m) => [...m, { role: "ai", ...reply(val) }]); }, 1000);
  };
        ...
        <div ref={scrollRef} className="ec-scroll" style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
          {messages.map((m, i) => (
            <div key={i} className="ec-fade" style={{ display: "flex", flexDirection: "column", alignItems: m.role === "user" ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "82%", padding: "11px 15px", borderRadius: m.role === "user" ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: m.role === "user" ? C.forest : C.paperDim, color: m.role === "user" ? "white" : C.ink, fontSize: 13.8, lineHeight: 1.55 }}>
                {m.text}
              </div>
              {m.actions && (
          ...
          {typing && <div className="ec-fade" style={{ alignSelf: "flex-start", background: C.paperDim, borderRadius: "14px 14px 14px 4px", padding: "11px 16px" }}><span className="ec-pulse" style={{ fontSize: 13, color: C.gray600 }}>digitando…</span></div>}
        </div>
        {messages.length < 2 && (
          <div style={{ padding: "0 16px 12px", display: "flex", gap: 8, flexWrap: "wrap" }}>
            {suggestions.map((s) => <button key={s} onClick={() => send(s)} className="ec-chip ec-focus" style={{ fontSize: 12.3, padding: "7px 12px", borderRadius: 999, border: `1.5px solid ${C.line}`, background: C.white, color: C.forest, fontWeight: 500 }}>{s}</button>)}
          </div>
        )}
...
/* ---- Planos (com checkout Pix real via /api/criar-pix) ---- */
function PlanosPage({ goto }) {
  ...
  const priceToNumber = (price) => Number(price.replace("R$", "").replace(".", "").replace(",", ".").trim());
    ...
    try {
      const res = await fetch("/api/criar-pix", {
        ...
        body: JSON.stringify({
          ...
          description: `Assinatura EcoCheck — Plano ${plan.name}`,
        }),
      });
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }} className="ec-plans-grid">
        {PLANS.map((p) => (
          <Card key={p.id} style={{ padding: 24, display: "flex", flexDirection: "column", border: p.highlight ? `2px solid ${C.green}` : `1px solid ${C.line}`, background: p.highlight ? C.greenSoft : C.white }}>
            <div className="ec-display" style={{ fontWeight: 700, fontSize: 16, color: C.forest }}>{p.name}</div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 4, margin: "12px 0 4px" }}>
              <span className="ec-display" style={{ fontSize: 26, fontWeight: 800, color: C.forest }}>{p.price}</span>
              <span style={{ fontSize: 13, color: C.gray600 }}>{p.period}</span>
            ...
            <div style={{ height: 1, background: C.line, margin: "14px 0" }} />
            <div style={{ display: "grid", gap: 9, flex: 1 }}>
              {p.features.map((f) => <div key={f} style={{ display: "flex", gap: 8, fontSize: 13.3, color: C.gray600 }}><Check size={15} color={C.green} style={{ flexShrink: 0, marginTop: 1 }} /> {f}</div>)}
            </div>
            <div style={{ marginTop: 18 }}>
              <Btn full variant={p.highlight ? "primary" : "secondary"} onClick={() => startCheckout(p)}>{p.id === "gratis" ? "Plano atual" : "Assinar"}</Btn>
            </div>
          </Card>
        ))}
      </div>

      {checkoutPlan && checkoutPlan.id !== "gratis" && (
        <div className="ec-fade" style={{ marginTop: 26 }}>
          <Card style={{ padding: 24, maxWidth: 420, margin: "0 auto" }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: C.forest, marginBottom: 4 }}>Pagamento via Pix — {checkoutPlan.name}</div>
            <div style={{ fontSize: 13, color: C.gray600, marginBottom: 16 }}>{checkoutPlan.price}{checkoutPlan.period}</div>
            {loadingPix && <div className="ec-pulse" style={{ fontSize: 13, color: C.gray600 }}>Gerando cobrança…</div>}
            {pixError && <div style={{ background: C.orangeSoft, color: C.orangeDeep, fontSize: 12.5, padding: "9px 12px", borderRadius: 9 }}>{pixError}</div>}
            {pix && (
              <div style={{ textAlign: "center" }}>
                {pix.encodedImage && <img src={`data:image/png;base64,${pix.encodedImage}`} alt="QR Code Pix" style={{ width: 200, height: 200, margin: "0 auto 14px" }} />}
                <div style={{ fontSize: 12, color: C.gray600, marginBottom: 6 }}>Pix copia e cola:</div>
                <textarea readOnly value={pix.payload} className="ec-input ec-mono" style={{ fontSize: 11, height: 70, resize: "none" }} onFocus={(e) => e.target.select()} />
              </div>
            )}
          </Card>
        </div>
      ...
      <div className="ec-mono" style={{ textAlign: "center", fontSize: 11.5, color: C.gray400, marginTop: 24 }}>
    ...
    <div>
      ...
      <Card style={{ padding: 24, maxWidth: 560 }}>
        <div style={{ display: "grid", gap: 14 }}>
          ...
          <Field label="E-mail para alertas"><input className="ec-input" defaultValue={currentUser?.email || ""} /></Field>
        </div>
...
export default function EcoCheckApp() {
  ...
  // Mantém o usuário logado ao recarregar a página (sessão do Supabase)
  // e carrega o diagnóstico salvo dele, se já tiver um.
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data }) => {
      const user = data.session?.user;
      if (user) {
        const meta = user.user_metadata || {};
        setCurrentUser({ name: meta.name || user.email, company: meta.company || "", email: user.email });
        const salvo = await carregarDiagnostico(user.id);
        setDiagnostic(salvo);
        setView("app");
        setPage("dashboard");
      }
  ...
  const goto = (dest, extra) => {
    if (dest === "auth") {
      setAuthMode(extra === "signup" ? "signup" : "login");
      setView("auth");
      window.scrollTo({ top: 0 });
      return;
    }
    if (["landing", "onboarding", "resultado"].includes(dest)) {
      setView(dest);
      window.scrollTo({ top: 0 });
      return;
    }
  ...
  // Chamado pelo Onboarding assim que a IA responde (usuário ainda pode não estar logado).
  const handleDiagnostic = (answers, diag) => {
    setPendingAnswers(answers);
    setDiagnostic(diag);
  };

  // Chamado após login/cadastro: se veio de um diagnóstico recém-gerado, salva no Supabase.
  // Se o usuário só entrou (sem diagnóstico novo), carrega o diagnóstico salvo dele.
  const handleAuthed = async (user) => {
    setCurrentUser(user);
    if (diagnostic && pendingAnswers) {
      await salvarDiagnostico(user.id, pendingAnswers, diagnostic);
  ...
  if (checkingSession) {
    return (
      <div className="ec-root">
        <style>{FONTS}</style>
        <FullScreenCenter>
          <Loader2 size={26} color={C.green} className="ec-spin" />
        </FullScreenCenter>
      </div>
    );
  }
  ...
  return (
    <div className="ec-root">
      ...
      {view === "onboarding" && <Onboarding goto={goto} onDiagnostic={handleDiagnostic} />}
      {view === "resultado" && <Resultado goto={goto} diagnostic={diagnostic} />}
      {view === "auth" && <AuthPage mode={authMode} setMode={setAuthMode} goto={goto} onAuthed={handleAuthed} />}
      {view === "app" && (
        <AppShell page={page} setPage={setPage} currentUser={currentUser} onLogout={handleLogout}>
          {page === "dashboard" && <DashboardHome goto={goto} currentUser={currentUser} diagnostic={diagnostic} />}
          {page === "diagnostico" && <Resultado goto={goto} diagnostic={diagnostic} />}
          {page === "pendencias" && <PendenciasPage goto={goto} diagnostic={diagnostic} />}
          {page === "documentos" && <DocumentosPage goto={goto} />}
          {page === "licencas" && <LicencasPage />}
          {page === "residuos" && <ResiduosPage />}
          {page === "obrigacoes" && <ObrigacoesPage />}
          {page === "calendario" && <CalendarioPage />}
          {page === "tutoriais" && <TutoriaisPage goto={goto} setActiveTutorial={setActiveTutorial} />}
          {page === "tutorial-detalhe" && <TutorialDetalhe goto={goto} />}
          {page === "assistente" && <AssistentePage goto={goto} />}
          {page === "planos" && <PlanosPage goto={goto} />}
          {page === "configuracoes" && <ConfiguracoesPage currentUser={currentUser} />}
        </AppShell>
      )}
    </div>
  );
